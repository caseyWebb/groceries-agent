...
└── users/                       ← one subtree per member (the tenant prefix)
    └── <username>/
        ...
        ├── grocery_list.toml  meal_plan.toml  cooking_log.toml  feeds.toml
...
- Write the initial `AGENT_INSTRUCTIONS.md` — orchestration guidance plus behavior rules.
...
- Create a "Grocery Agent" Project in Claude.ai. Paste `AGENT_INSTRUCTIONS.md` into project instructions.
...
- Implement the menu-generation flow conversationally — Claude orchestrates tool calls based on AGENT_INSTRUCTIONS.md guidance. Iterate on the instructions as the LLM does the wrong thing.
...
- Update `AGENT_INSTRUCTIONS.md` to instruct Claude on the comprehensive pantry confirmation pass (including staples for drift catching), the sequencing pass, and the substitution timing split.
...
- Update `AGENT_INSTRUCTIONS.md` so Claude proactively asks before editing `taste.md` (user-curated rule).
...
- Update `AGENT_INSTRUCTIONS.md` so menu generation honors principles in proposals, explaining tradeoffs when it can't satisfy all of them.
...
- **MCP prompts vs. project instructions:** if the orchestration narrative in AGENT_INSTRUCTIONS.md grows unwieldy, would moving some flows into MCP prompts help? Defer until felt.
...
- **Phase 3:** Agent lives in Claude.ai. Menu requests work end-to-end. SKU cache populates. Some bumps as `AGENT_INSTRUCTIONS.md` gets tuned.
