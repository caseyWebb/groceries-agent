...
| `propose_meal_plan` / `display_meal_plan` | `new_for_me: string[]` | **accepted and ignored** — nothing resolved, nothing force-placed, never an error; discovery seasoning is engine-internal now (the palette path derives its own seed from the same data `list_new_for_me` reads) | `retired` (superseded by the engine-internal derivation) |
...
**Removal condition (the `single-slot-discovery` row):** the `new_for_me` accept-and-ignore row above is removed once **both** hold: a subsequent plugin publish has occurred **and** ≥30 days have elapsed since that publish (the `single-slot-discovery` persona republish — which drops `list_new_for_me` from the `plan` flow's context reads and fold-in guidance — starts the clock). After the window, `new_for_me` falls through to the generic unknown-key rejection like any other unrecognized param.
