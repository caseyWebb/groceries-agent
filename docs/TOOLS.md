...
- `relist_grocery_send_line(send_id, line_key, expected_row_version)` performs guarded `in_cart → active`, clears linkage, retains immutable history, and writes no spend. A non-null `send_id` must name the row's current open send. Null is accepted only when the row is proven not to belong to any current open send, including an unlinked, dangling, or already-placed linkage.
...
**Params:**
- `update_preferences`: `patch` (object, required) — a **JSON Merge Patch (RFC 7396)** over the caller's preferences: a present key sets/overwrites, `null` deletes, nested objects merge to **any depth**, arrays replace wholesale. Only the keys you touch change — a partial patch never clobbers siblings, so you do **not** re-send the whole object. Defined top-level keys: `cadence` (the per-meal planning-frequency map `{ breakfast?, lunch?, dinner? }`, each an integer weekly count **0–7**, merged **per key** consistently with the RFC 7396 contract — `{ cadence: { lunch: 2 } }` sets lunch only, `{ cadence: { dinner: null } }` clears one meal, `cadence: null` clears the map; never a wholesale replacement. Drives `propose_meal_plan`'s per-meal slot counts), `planning_cadence_days` (positive number — how far out the caller plans/shops, in days; drives `propose_meal_plan`'s weather horizon and vibe-recurrence caps, unset falls back to a 7-day window), `weekly_budget` (number ≥ 0 — the household's weekly grocery budget in **dollars/week**, echoed by `retrospective`'s spend section; unset or `0` means "no budget line"; a negative or non-numeric value is `malformed_data`; `null` deletes back to unset), `stores` (`{primary, preferred_location, location_zip}`), `brands` (map of ingredient-family term → **tier object** `{ tiers?: string[][], any_brand?: boolean }` — `tiers` is an ordered ladder: the top tier is tried first, brands within a tier are equally acceptable (cheapest wins), later tiers are fallbacks; `any_brand: true` = when the tiers (if any) are exhausted, take the cheapest acceptable instead of asking, so `{ any_brand: true }` alone is the standing don't-care; family absent = ambiguous/ask; `null` = clear back to ambiguous. A family value is itself merged, so a partial patch like `{ any_brand: true }` preserves the stored tiers; a brand may appear in only one tier of a family, and the all-empty `{ tiers: [], any_brand: false }` is rejected — `null` is the way to clear. For **one deprecation window** a legacy flat rank list is accepted and converted — see the deprecation convention above), `dietary` (`{avoid[], limit[]}`), `rotation` (`{resurface_after_days, novelty_boost}` — tunes the `search_recipes` freshness re-rank; both positive numbers). Anything else nests under `custom`; an unknown top-level key returns `validation_failed` (nest it under `custom`). **One-deprecation-window shims** (see the convention above, checked BEFORE the unknown-key rejection): `default_cooking_nights: N` (validated int 0–7) is accepted as an **alias merged onto `cadence.dinner`** — breakfast/lunch preserved, the frozen column never written — with a `{ key: "default_cooking_nights", reason: "aliased", superseded_by: "cadence.dinner" }` warning; the **retired** `lunch_strategy` / `ready_to_eat_default_action` are **accepted and dropped** — not validated, not stored, not routed to `custom`, never `validation_failed`, never the nest-under-`custom` hint — each with a `{ key, reason: "retired", superseded_by: "meal vibes" }` warning (capture that intent as lunch/dinner meal vibes instead). A type-invalid merged result returns `malformed_data` and stores nothing. Applied atomically to the D1 `profile` row + `brand_prefs` rows.
