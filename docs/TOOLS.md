...
- `id` (string, optional) — an **already-canonical** ingredient id (e.g. `cabbage::color-red`). When supplied the row keys on it **directly** — validated as a canonical id, **not** re-resolved through the normalization funnel — and its `display_name` is taken from the identity node's `display_name` (or an explicitly supplied display), so a caller (e.g. the app materializing an accepted graph-sibling swap) pins a row that dedups/joins exactly while rendering a clean label. It dedups against any existing row on that id (a merge keeps the surviving row's display). An invalid/unknown id falls back to resolving `name`.
