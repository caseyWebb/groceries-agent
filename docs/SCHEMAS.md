...
tenant           TEXT  -- owning user
name             TEXT  -- surface form / order-time search-term input; an add-by-id row stores the canonical id here
display_name     TEXT  -- optional explicit label override (usually NULL); the label is otherwise resolved at read
...
- The rendered human label is resolved as: the row's explicit `display_name` override when set; else, for an **id-named** row (its `name` equals its canonical `normalized_name` — an **add-by-id** row stores the canonical id as its `name`, and **plan-derived** virtual lines are id-named), the identity node's label (its `display_name`, else the `base (detail)` synthesis), resolved at read so it converges as the node is curated/backfilled (a legacy id-named row heals with no row edit); else the stored `name` (a typed add's member phrasing, rendered verbatim). `display_name` is nullable and rarely populated — it is the highest-precedence override, not the primary label source. A merge (a re-add or an add-by-id landing on an existing row) keeps the surviving row's `name`/`display_name`, never adopting the incoming surface form.
