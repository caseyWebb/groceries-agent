...
The reconcile-owned home of each recipe's **derived** fields (migration 0013, formerly `recipe_embeddings`). Not authored, not frontmatter, not projected by the build — written only by the scheduled reconcile (`src/recipe-embeddings.ts`) and the import-time seed (`create_recipe`). Keyed by `slug`; the build's wholesale `recipes` rebuild never touches it (different producer + cadence — the reason it is a sibling table).
...
- `GET /health.svg?token=<HEALTH_TOKEN>` → the same aggregate payload rendered as an SVG **card** (`content-type: image/svg+xml`) for a README badge (data-repo-health-badge). Same token gate (404 unset, 401 wrong), but **always HTTP 200** — degraded state is shown by color, not status, because an image proxy (GitHub Camo) may not render a non-200 as an image — with a short `Cache-Control` so it refreshes on a TTL. Tenant-data-free, like `/health`; a never-run job renders amber (pending), not red. It's a glance, not an alarm: point real HTTP-status/freshness monitors at `/health` (JSON), not `.svg`.
