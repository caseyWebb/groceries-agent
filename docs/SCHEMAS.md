...
tenant           TEXT  -- owning user
name             TEXT  -- surface form / resolver input (e.g. "olive oil")
display_name     TEXT  -- human-facing label, or NULL; a human surface renders display_name ?? name
...
- `display_name` (nullable) is the human-facing label, stored independently of both the resolver-input `name` and the canonical `normalized_name`; a human surface renders `display_name ?? name`, never the raw canonical id. A merge (adding "green onions" onto an existing "scallions" row on the same canonical id) keeps the surviving row's display (`name`/`display_name`), never overwriting it with the incoming surface form.
...
tenant           TEXT  -- owning user
name             TEXT  -- surface form / order-time search-term input (required)
display_name     TEXT  -- human-facing label, or NULL; a human surface renders display_name ?? name
...
- `display_name` (nullable) is the human-facing label, stored independently of both the resolver-input `name` and the dedup key `normalized_name`; a surface a human reads renders `display_name ?? name`, never the raw `normalized_name`/id. It is set for **add-by-id** and **plan-derived** rows — the no-member-phrasing cases — from the identity node's `display_name` (a row keyed `cabbage::color-red` renders "Red cabbage"); a typed add leaves it NULL and renders the member's `name` verbatim. A merge (a re-add or an add-by-id landing on an existing row) keeps the surviving row's display, never adopting the incoming surface form.
...
**The `local_rejects` wire field (the local-reject reporting path)** — the loudest breakage never reaches a Worker-side-only ledger: the satellite's own validators (`validateSaleEmit`/`validateOrderEmit`/the recipe adapter's contract check) drop a malformed or judgment-smuggling item **before the wire**. So the satellite attaches a compact, **pre-aggregated** local-reject summary to each of its three delivery envelopes — the push batch (`SatelliteBatchSchema` / `POST /admin/api/ingest`), the pull-channel results (`POST /satellite/results`), and the order-receipt (`POST /satellite/order/receipt`) — and the Worker records each entry as one `origin: local` ledger row. The field is defined **once** in `@grocery-agent/contract` and is **additive + OPTIONAL** on all three envelopes, so it keeps `CONTRACT_VERSION: "v2"` (a satellite that omits it is unaffected; an older Worker ignores an unknown field):
