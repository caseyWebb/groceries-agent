...
One read tool covers the whole umbrella: `read_guidance(domain?, slugs?)` — omit `slugs` to list a domain's entries (or all domains grouped), name them to read content (`list_guidance` remains a one-window dispatch alias). There is no agent write path: every domain is operator-curated by editing the vault (the Obsidian vault synced to the R2 bucket); the admin panel's Data › Guidance area is a read-only viewer. See `docs/TOOLS.md`.
...
- **Curated config, edited by hand** — the domain changes only by editing the corpus (the Obsidian vault synced to the R2 bucket), the same posture as every guidance domain.
...
- **Shared, operator-curated**: one shared tree read by the whole group, written only by vault edits through the R2 sync (`src/corpus-store.ts` is the Worker's read path).
...
- **Shared, operator-curated** (the `cooking_techniques` posture): one shared tree read by the whole group, written only by vault edits.
