...
A member's **attributed notes** on one recipe (shared or personal) — the spin-capture mechanism. Stored in the D1 `recipe_notes` table (`id TEXT PRIMARY KEY`); columns: `recipe` (slug), `author` (the writing member id, set by the Worker; founding member = tenant id for pre-split rows), `body`, `tags`, `private`, `created_at`. Append-mostly. Adding a note never modifies shared content; an author MAY edit or delete their **own** notes (`update_recipe_note` / `remove_recipe_note`, addressed by `created_at`, self-scoped) but never another tenant's.
