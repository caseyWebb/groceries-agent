...
- `webauthn:chal:<challenge>` → the pending challenge record (the ceremony kind — a registration for a specific enrolling tenant, or a discoverable-login challenge carrying no tenant). **Single-use** — deleted on verification — with a short TTL (~5 min). A returned attestation/assertion is verified against the stored challenge; a missing or already-consumed challenge fails verification.
- `authz:<ref>` → `{ oauth_req, status, verify_code, tenant? }` — the Claude.ai `/authorize` **approval reference**. `oauth_req` is the base64-encoded parsed OAuth request; `status` is `"pending"` → `"approved"` → consumed; `verify_code` is the short human-readable code shown on both the `/authorize` page and the web app's `/connect` screen; `tenant` is bound server-side when a passkey-authenticated member approves. **Single-use** with a ~10-min TTL — the poll completes `completeAuthorization` EXACTLY ONCE and then marks the reference consumed; an expired or already-consumed reference is rejected and mints no token.

## webauthn_credentials (per-tenant, D1 `webauthn_credentials` table)

Each member's enrolled **passkeys** (passkey-auth) — one row per device, many per tenant. A member authenticates on both member surfaces with these credentials; the invite code is only the single-use bootstrap that seeds their first login/connection (see *Invite records* above). Written/read through `src/db.ts` (never `env.DB` directly), keyed by the credential id; `idx_webauthn_tenant` on `(tenant)` backs the list-by-tenant read and the revocation purge. Included in the per-tenant `TENANT_TABLES` batch (`src/admin.ts`), so member revocation deletes every row. Passkey login and connect-approve are rate-limited per client IP by the shared fixed-window limiter (`src/rate-limit.ts`, fail-open). Migration `0042_webauthn_credentials`.
