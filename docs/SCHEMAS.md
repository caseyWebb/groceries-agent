...
- **Resource usage** (usage-observability) → `{ configured: true, generated_at, day, kv: { limits, totals, namespaces: [{ namespace_id, read, write, delete, list }] }, ai: { neurons_limit, neurons_used, by_model: [{ model, neurons }] } }` — the current UTC day's account-wide KV operations + Workers AI neurons against the daily free-tier limits, read from the Cloudflare GraphQL Analytics API. `kv.limits`/`kv.totals` are `{ read, write, delete, list }`. Performs **no KV** (observing the budget must not consume it); tenant-clean (account/namespace aggregates only); KV rows keyed by namespace **id**.
...
The **Data** area is **server-rendered** (`src/admin-data.ts`, navigated by query params — no JSON API): read-only, cross-tenant views over D1 and the R2 corpus, behind the same Access gate (so they 404 with the rest of the surface when it is disabled). Each view runs a **fixed** query (no operator-supplied SQL), goes through `src/db.ts` / `src/corpus-store.ts`, and performs **no redaction** — `private` notes are shown and cross-tenant aggregates name their tenants. Scope is D1 domain data + the R2 corpus only; no KV secret is reachable. The readers produce these shapes, which the pages render:
...
- **recipe detail** (`<slug>`) → the cross-tier record: `{ slug, status, reconcile_message, source, body, projection, derived: { description, has_embedding, state } | null, dispositions: [{ tenant, favorite, reject }], notes: [...] }`. `status` is derived from (R2 source present?, `recipes` row present?); `skipped` carries the `reconcile_errors` reason; the embedding is shown as presence only, never the raw vector. `body` is `source` with its YAML frontmatter fence removed (the whole text when there's none, `null` when there's no source). `not_found` when the slug is in neither tier.
- **member detail** (`<id>`) → one member's full per-tenant state: `{ id, profile, pantry, meal_plan, grocery_list, overlay, cooking_log, recipe_notes, store_notes }`. The id is resolved against the allowlist (`not_found` when absent); reuses the canonical `profile-db`/`session-db` readers.
- **corpus table** (`<table>`) → `{ table, columns, rows }` for an allowlisted shared-corpus table (`aliases`, `flyer_terms`, `feeds`, `stores`, `store_notes`, `sku_cache`). `sku_cache` is bounded by a default `LIMIT`.
- **guidance browse** → list a `guidance/**` R2 prefix as `{ prefix, entries: [{ name, type }] }`; one object as `{ key, markdown }`.
- **discovery table** (`<table>`) → `{ table, columns, rows }` for `discovery_candidates` / `discovery_senders` / `discovery_members` / `discovery_rejections`.
- **system table** (`<table>`) → `{ table, columns, rows }` for `reconcile_errors` / `bug_reports` / `schema_meta` (where agent-filed bug reports are surfaced).
