...
- **`/health` aggregates them** on the **fetch** path — deliberately, because `fetch` is independent of `scheduled`, so the endpoint stays answerable when the cron is dead, and an external monitor catches a stopped job via stale `last_run_at`. It is token-gated (`HEALTH_TOKEN`; 404 when unset) and aggregate-only (no per-tenant data; 200 when ok, 503 when a job is failing). A sibling **`/health.svg`** renders the same payload as a README status badge (same token gate, but always 200 + `image/svg+xml` so an image proxy will render it; degraded shows by color). `HEALTH_TOKEN` may be supplied as a `var` rather than a secret so the deploy can stamp the badge into the (private) data-repo README — see operator-provisioning.
