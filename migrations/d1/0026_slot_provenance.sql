...
-- slot it was proposed to fill (`from_vibe`); when the recipe is cooked, `log_cooked` copies
-- that onto the `cooking_log` row (`satisfied_vibe`) in the SAME transaction that clears the
-- plan, so the cadence scheduler can attribute satisfaction back to the vibe that shaped the
-- slot ("shape in → shape out"). Both columns are additive + optional: absent for a
-- hand-picked / off-vibe plan, and behavior-preserving when null.
