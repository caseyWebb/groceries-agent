#!/usr/bin/env node
// dev-admin.mjs — `aubr dev:admin`: the admin panel's HMR dev loop (admin-spa).
// Spawns `wrangler dev` (packages/worker — the real Worker: Access gate, /admin/api, data) and
// `vite` (packages/admin-app — HMR at :5174, whose server.proxy carries /admin/api to :8787 so
...
function run(name, bin, args, cwd) {
  ...
  child.on("exit", (code, signal) => {
    ...
    console.error(`[dev-admin] ${name} exited (${signal ?? `code ${code}`}) — shutting down`);
...
run("vite", "vite", [], path.join(ROOT, "packages", "admin-app"));
