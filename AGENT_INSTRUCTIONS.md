...
1. **Load context, then distill the request into search specs.** Fire the same parallel context batch as `meal-plan` step 1 — `read_user_profile`, `read_pantry`, `retrospective("month")`, `fetch_rss_discoveries`, `read_discovery_inbox`, `get_weather_forecast` (and `kroger_flyer` only if my store is Kroger) — **but do NOT load `list_recipes()`**: retrieval replaces the whole-corpus dump. Then turn my request + that context into a handful of **search specs** for `recipe_semantic_search`, each `{ vibe, facets, label }`:
