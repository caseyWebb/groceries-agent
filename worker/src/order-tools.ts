...
export function registerOrderTools(
  ...
  server.registerTool(
    ...
    {
      description:
        "Order-time flush: resolve the whole grocery_list (∪ menu_needs − pantry_has) against current Kroger availability, write the cart (PUT /v1/cart/add), and append learned SKU mappings to skus/kroger.toml. Ambiguous/unavailable items return as a single `checkpoint` (NOT added) for the user to disposition; pantry overlaps return as `partials` to prompt on. Resolved items advance to status:in_cart only after a successful cart write. SKU-cache commit and cart write are independent best-effort — partial status is reported honestly; the cart is never reported populated when its write failed (check `cart.code` for `reauth_required`). The ONLY tool that writes a Kroger cart. Default buy = 1 package per item; set a count via `menu_needs[].quantity` (or the `quantities` map, which overrides it). Lines that defaulted to 1 are returned with `assumed_quantity: true` — at preview, reconcile by-the-each produce against the recipe before flushing. preview=true resolves and reports without writing anything.",
      inputSchema: {
