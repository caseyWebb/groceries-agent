...
- **New frontend packages ride the native toolchain.** `packages/app`/`packages/admin-app`/`packages/ui` lint with **oxlint** and format with **Biome** — no ESLint or Prettier in the new packages; typecheck stays `tsc` (each package has a `typecheck` script the recursive root pass picks up).
