...
export function registerDiscoveryTools(server: McpServer, sharedGh: GitHubClient): void {
  ...
  server.registerTool(
    ...
    {
      description:
        "Write a NEW recipe to the SHARED corpus, as one solo commit. Slug derives from the title unless `slug` is given. Discovery imports: pass status 'draft' with discovered_at + discovery_source (status defaults to 'draft' if omitted). The body MUST contain ## Ingredients and ## Instructions. Classify `requires_equipment` conservatively: default [] (the common case) and include a vocab slug (pressure-cooker | sous-vide-circulator | blender | ice-cream-maker) ONLY when the dish is genuinely impossible without it — a wrong tag silently hides a makeable recipe. `perishable_ingredients` lists the ingredient names that would spoil before use (the \"would the leftover rot\" test) — fuzzy edges (eggs, potatoes, hardy roots) are fine to skip; default []. Refuses to overwrite an existing slug (slug_exists), and refuses to duplicate a recipe whose `source` URL is already in the corpus (already_exists, with the existing slug — reuse it).",
      inputSchema: {
