...
4. **Substitutions pass.** With the preview in hand, call `suggest_substitutions` for the to-buy lines (it defaults to the same set; over ~12 lines, follow the returned `remaining` with another call) and present what it found alongside the checkpoint batch: cheaper same-item picks (real unit prices ride the result — cite them), genuine sale swaps, in-stock alternatives for anything whose current pick is out of stock, and the relation-labeled cross-ingredient siblings (a sibling flagged `in_pantry` means I may not need to buy at all). The tool proposes and *names* each relation — whether a sibling actually fits the dish, and any "lower fat"-style judgment, stays **yours**, grounded in its data; skip weak suggestions rather than reciting them. Nothing is applied by the tool: map what I accept onto the flush — a **same-item swap** (different SKU) becomes an `overrides: [{ name, sku }]` entry; a **cross-ingredient swap** on an explicit row is `add_to_grocery_list` (note the swap) + `remove_from_grocery_list`; on a plan-derived line it's the add **plus** an order-scoped `exclude` of the original (the plan still lists it). What I decline, drop without comment.

5. **Flush.** Once I've dispositioned the batch, call `place_order` for real — pass `overrides` for the items I picked SKUs for (including accepted same-item substitutions), `include_partials` for the partials I confirmed, `quantities` for anything beyond 1 package, and `exclude` for any line I said to skip this order ("don't buy the salmon this time" — a derived line has no row to remove, and the exclusion is order-scoped, never persisted). Resolved items advance to `in_cart`.
