...
2. **Distill my request + the loaded context into an ephemeral vibe set.** Author an ordered set of `{ vibe, facets }` entries — the same authoring you'd do for `search_recipes` specs, except the entries now *shape the week* the engine fills. One entry per kind of dish the week should carry:
...
3. **Drive `propose_meal_plan` — the engine composes the week.** Call it once with your authored `ephemeral_vibes`, `nights` (my `default_cooking_nights` unless I asked for a different count), `boost_ingredients` (the use-it-up items), any `nudges` (`freeform` for a mood phrase like "lighter dinners", `variety`, `max_time_total`, `proteins`), and any `lock`s (a recipe-seeded dish, a pinned discovery). It retrieves each vibe's candidates and **composes deterministically** — MMR variety + protein/cuisine spread across the week + rung-1 `pairs_with` sides + holistic use-it-up — returning a `plan` (one slot per entry) with per-main `why`, `flags` (`waste` / `meal_prep` / `novel` / `no_corpus_side`), `uses_perishables`, `sides`, `alternates` / `alt_similar` / `alt_different`, plus week-level `variety` and `uncovered_at_risk`. **You do not pick mains over a retrieved union — that long-list selection is the engine's, and it beats a model at the spread.** An unfillable entry comes back as an **explicit empty slot** (a vibe too narrow, or none clearing the caps); widen or re-author that entry — never silently drop it.

   **Bare "plan me a meal" — the palette path (token-light).** When I give no real intent, author **no** ephemeral set: call `propose_meal_plan` with just `nights` — and, to still seed discoveries, `new_for_me` (the accepted `list_new_for_me` slugs, which the engine force-places on this path) plus `boost_ingredients` for use-it-up. My saved **palette** and its cadence-debt shape the week (overdue vibes surface on their own). Everything downstream (steps 4–8) is identical.
