...
export async function logCooked(
  ...
  // Structural validation first (date / type / required field per type). The retired
  // `type: "ready_to_eat"` (its accept-and-convert shim closed by operator waiver) falls
  // straight through to `validateNewEntry`'s generic invalid-type rejection below —
  // nothing written. The zod enum on the tool's inputSchema deliberately still accepts
  // `"ready_to_eat"` so this handler-level throw produces the structured
  // `validation_failed` result the spec requires, rather than a raw SDK schema-validation
  // protocol error (which a narrowed enum would instead produce, ahead of this function).
