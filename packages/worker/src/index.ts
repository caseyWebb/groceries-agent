...
export default {
  ...
  async scheduled(_controller: ScheduledController, env: Env, _ctx: ExecutionContext): Promise<void> {
    ...
    // archetypes on the small model and enqueues add_vibe proposals), and the corpus
    // dup-scan (recipe-dedup): bounded + watermarked pure arithmetic over the phase-2
    // projection's fresh ingredients_key and the phase-3 reconcile's fresh vectors (the
    // same freshness ordering the sweep relies on), surfacing near-duplicate pairs as
