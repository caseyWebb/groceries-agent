...
describe("log_cooked (D1, transactional meal-plan clear)", () => {
  ...
  it("rejects an unknown type (validation_failed), including the retired ready_to_eat", async () => {
    const d1 = fakeD1({ recipes: [] });
    const handlers = collect(d1.env, "everett");
...
// The retired type's accept-and-convert shim is closed (operator waiver, close-cull-windows):
// a stale plugin's type: "ready_to_eat" write is rejected as validation_failed like any other
// unknown type — nothing written. The tool's inputSchema still enumerates "ready_to_eat" so
// this rejection is a structured handler-level throw (runTool's normal path), never a raw SDK
// schema-validation protocol error. Historical rows already stored with type = 'ready_to_eat'
// are untouched and keep aggregating exactly as before (retrospective.test.ts, cooking-tools.test.ts).
describe("log_cooked — the retired ready_to_eat type is rejected", () => {
  it("rejects a ready_to_eat write with validation_failed, writing nothing", async () => {
    const d1 = fakeD1({ recipes: [] });
    const handlers = collect(d1.env, "everett");
    const out = parse(
  ...
  it("rejects it regardless of meal or plan context — no plan row is ever touched", async () => {
    const d1 = fakeD1({
      recipes: [],
    ...
    expect(out.error).toBe("validation_failed");
    expect(d1.batches).toHaveLength(0);
    expect(d1.tables.meal_plan).toHaveLength(1); // untouched
  });

  it("rejects it via the shared logCooked op directly too — the same structured throw the route/tool both surface", async () => {
    const d1 = fakeD1({ recipes: [], tables: { cooking_log: [] } });
    await expect(
      logCooked(d1.env, "everett", { type: "ready_to_eat", name: "frozen lasagna", date: "2026-06-20" }, { dedupe: true }),
