...
describe("rejectUnknownPatchKeys", () => {
  ...
  it("the retired keys are unknown top-level keys — rejected like anything else (no shim intercepts them)", () => {
    expect(() => rejectUnknownPatchKeys({ lunch_strategy: "mixed" })).toThrowError(/custom/);
    expect(() => rejectUnknownPatchKeys({ ready_to_eat_default_action: "auto-add" })).toThrowError(/custom/);
...
describe("effectiveCadenceCount / exportPreferences (the read-side chain)", () => {
  it("stored cadence[meal] wins; else dinner derives from the fixed default 5; breakfast/lunch derive 0", () => {
    expect(effectiveCadenceCount({ cadence: { dinner: 3 } }, "dinner")).toBe(3);
    expect(effectiveCadenceCount({}, "dinner")).toBe(5);
    expect(effectiveCadenceCount(null, "dinner")).toBe(5);
    expect(effectiveCadenceCount({}, "lunch")).toBe(0);
    expect(effectiveCadenceCount({ cadence: { lunch: 2 } }, "lunch")).toBe(2);
  });

  it("exportPreferences: cadence is the stored map, else the fixed derivation", () => {
