...
function buildBody(fields, draft) {
  const body = {};
  for (const f of fields) {
    const raw2 = (draft[f.key] ?? "").trim();
    if (!raw2 && !f.required) continue;
    if (f.kind === "number") body[f.key] = Number(raw2);
    else if (f.kind === "tags") body[f.key] = raw2 ? raw2.split(",").map((s) => s.trim()).filter(Boolean) : [];
    else body[f.key] = raw2;
  }
  return body;
}
function summarize(r) {
  ...
  return r.sample.length ? `${feed} \xB7 sample: ${sample}` : feed;
}
async function readErr(res) {
  const b = await res.json().catch(() => null);
  return b?.message ?? `HTTP ${res.status}`;
}
function CorpusEditor({ config, page }) {
  const [rows, setRows] = useState(page);
  const [draft, setDraft] = useState({});
  const [action, setAction] = useState({ t: "idle" });
  const [test, setTest] = useState({ t: "none" });
  ...
  const testCell = (key, url) => /* @__PURE__ */ jsxDEV("span", { class: "log-actions", children: [
    /* @__PURE__ */ jsxDEV("button", { class: "link", disabled: test.t === "testing", onClick: () => runTest(url, key), children: test.t === "testing" && test.key === key ? "testing\u2026" : "test" }),
    test.t === "result" && test.key === key ? /* @__PURE__ */ jsxDEV("span", { class: test.ok ? "muted small" : "small", style: test.ok ? "" : "color:var(--danger)", children: test.summary }) : null
  ] });
  return /* @__PURE__ */ jsxDEV("div", { children: [
    action.t === "failed" ? /* @__PURE__ */ jsxDEV("div", { class: "error", children: [
      action.op.op === "add" ? "Add" : "Remove",
      " failed: ",
      action.error
    ] }) : null,
    /* @__PURE__ */ jsxDEV("table", { children: [
      /* @__PURE__ */ jsxDEV("thead", { children: /* @__PURE__ */ jsxDEV("tr", { children: [
        rows.columns.map((c) => /* @__PURE__ */ jsxDEV("th", { children: c })),
        /* @__PURE__ */ jsxDEV("th", {})
      ] }) }),
      /* @__PURE__ */ jsxDEV("tbody", { children: rows.rows.map((row) => {
        const key = String(row[config.pkColumn] ?? "");
        return /* @__PURE__ */ jsxDEV("tr", { children: [
          rows.columns.map((c) => /* @__PURE__ */ jsxDEV("td", { class: "small", children: cell(row[c]) })),
          /* @__PURE__ */ jsxDEV("td", { class: "form-actions", children: [
            config.testUrlColumn ? testCell(key, String(row[config.testUrlColumn] ?? "")) : null,
            /* @__PURE__ */ jsxDEV("button", { class: "danger", disabled: busy, onClick: () => remove(key), children: busy && action.t === "busy" && action.op.op === "remove" && action.op.key === key ? "removing\u2026" : "remove" })
          ] })
        ] });
      }) })
    ] }),
    /* @__PURE__ */ jsxDEV("div", { class: "card", children: [
      /* @__PURE__ */ jsxDEV("h2", { children: "Add" }),
      config.addFields.map((f) => /* @__PURE__ */ jsxDEV("label", { children: [
        f.label,
        f.required ? "" : " (optional)",
        /* @__PURE__ */ jsxDEV(
          "input",
      ...
      /* @__PURE__ */ jsxDEV("div", { class: "form-actions", children: [
        /* @__PURE__ */ jsxDEV("button", { disabled: busy, onClick: add, children: busy && action.t === "busy" && action.op.op === "add" ? "adding\u2026" : "add" }),
        config.testUrlColumn ? /* @__PURE__ */ jsxDEV("button", { class: "link", disabled: test.t === "testing", onClick: () => runTest(draft[config.testUrlColumn] ?? "", "__draft__"), children: "test url" }) : null
      ] }),
      config.testUrlColumn && test.t === "result" && test.key === "__draft__" ? /* @__PURE__ */ jsxDEV("p", { class: test.ok ? "muted small" : "small", style: test.ok ? "" : "color:var(--danger)", children: test.summary }) : null
    ] })
  ] });
}
var host = document.getElementById("config-island");
var propsEl = document.getElementById("config-props");
if (host && propsEl) {
  const props = JSON.parse(propsEl.textContent ?? "{}");
  host.replaceChildren();
  render(/* @__PURE__ */ jsxDEV(CorpusEditor, { config: props.config, page: props.page }), host);
}
