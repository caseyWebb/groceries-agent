...
// src/admin/client/opconfig.tsx
var client = hc(location.origin);
function shown(config, f) {
  const v = config[f.key];
  return String(f.pct ? Math.round(v * 100) : v);
}
function OpConfigIsland({ config, fields }) {
  const initial = {};
    ...
    fields.map((f) => /* @__PURE__ */ jsxDEV("label", { children: [
      ...
      /* @__PURE__ */ jsxDEV(
        "input",
        {
          ...
          step: f.step,
          value: draft[f.key],
          onInput: (e) => onField(f.key, e.target.value)
        }
      )
    ] })),
...
if (host && propsEl) {
  ...
  render(/* @__PURE__ */ jsxDEV(OpConfigIsland, { config: props.config, fields: props.fields }), host);
}
