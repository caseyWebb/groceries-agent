...
- **THEN** the carted and substituted lines advance to `in_cart` (same keying as `place_order`) and the unavailable lines remain `active`
...
- **THEN** the issued `in_cart` lines advance to `ordered`; absent the signal they stay `in_cart`, identical to an unconfirmed Kroger cart
