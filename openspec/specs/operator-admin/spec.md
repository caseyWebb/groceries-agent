...
**The single exception is `POST /admin/api/ingest`** (see `recipe-ingestion`): this one route SHALL be an explicit, allowlisted exemption from the Access gate, authenticated instead by a bearer **ingest key**, because a headless home-network satellite carries no Access JWT. The exemption SHALL be scoped to that exact path and method; every other `/admin*` path SHALL remain Access-gated. A request presenting an ingest key to any other `/admin*` path SHALL be rejected `403` by the Access gate (the ingest key is not admin credentials).
...
- **WHEN** a satellite POSTs `/admin/api/ingest` with a valid ingest key and no Access assertion
