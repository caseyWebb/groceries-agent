...
The operation SHALL compare the requested keys with the complete eligible set and the rendered snapshot before deriving effects. Its atomic claim SHALL then recheck the complete eligible rows plus the bounded state that can change their membership (grocery rows, pantry, plan, substitution/coverage decisions, and the explicitly requested walk store/visible notes); enriched-only Grocery inputs such as quote/corpus display metadata are preflight context, not an unbounded transactional signature. A bounded-state difference SHALL return `checked_set_changed` with fresh authoritative state and SHALL perform no partial deletion, restock, or spend write. Checked virtual plan needs SHALL already exist as canonical `source:'menu'` rows through the checked operation; shop commit SHALL NOT materialize virtual lines.
