...
A corpus edit made outside the Worker (e.g. an Obsidian sync to R2) SHALL be validated by the reconcile, not by CI. When such an edit is invalid, the system SHALL surface the failure through an agent-readable record and operator-visible health/notification, rather than silently dropping it. The system SHALL NOT require a GitHub CI run to validate corpus content.

#### Scenario: A bad Obsidian edit is surfaced, not silent

- **WHEN** an author syncs a malformed recipe to R2 via Obsidian
...
#### Scenario: A valid Obsidian edit indexes without CI

- **WHEN** an author syncs a well-formed recipe edit to R2
