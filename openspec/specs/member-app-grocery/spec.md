...
The launcher SHALL keep the manual-shop fallback outside the adapter projection. A configured Instacart Marketplace handoff SHALL appear as its own enabled launcher entry; an unconfigured adapter SHALL produce no entry. A Satellite entry with unavailable freshness SHALL be disabled, and an Offline walk entry SHALL identify the selected existing store without duplicating its identity.
...
#### Scenario: Instacart launcher follows configured availability

- **WHEN** the Profile card reports Instacart available from complete valid operator configuration
- **THEN** the grocery launcher contains one `marketplace_handoff` entry, and absent or invalid configuration produces no Instacart entry
