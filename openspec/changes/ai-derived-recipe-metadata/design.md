...
6. **Model choice is a tunable with an eval, not a guess.** A Workers AI instruct model (e.g. Llama-class) writes the description. The task is short, constrained, and low-stakes; cost at friend-group volume (a few imports/week, change-driven steady state) is trivial and well inside the Workers AI allowance. A quality eval (below) gates the model pick; the model id is config, swappable without a contract change.
