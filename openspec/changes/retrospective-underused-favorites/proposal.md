...
- **Pin a canonical season vocabulary** — `spring | summer | fall | winter` — with read-side normalization (`autumn → fall`, case-folded) so the season match is reliable against legacy free-text values. Write-side canonicalization (importer/classifier guidance, optional validator check) is staged as a follow-on so the feature ships without an importer change or data migration.
...
- `recipe-metadata-contract`: the `season` field gains a **canonical vocabulary** (`spring | summer | fall | winter`) that read paths normalize against; presence/empty-array rules are unchanged.
...
- **Staged follow-on (not blocking):** write-side season canonicalization — classifier guidance in `AGENT_INSTRUCTIONS.md` and an optional soft check in `src/recipe-contract.js`.
