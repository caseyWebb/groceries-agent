...
- **Migration**: none required for operators — a deep merge with code as the base adds code's missing keys (e.g. `triggers`) while the operator's existing keys (KV ids, vars, routes) still win, so existing full-copy `wrangler.jsonc` files keep working. A later cleanup could slim operator configs to just their overrides (out of scope).
- **Risk**: a wrong merge could mis-bind KV (cross-tenant) or mis-route — so the KV-id-provenance rule and the merge helper's tests are the heart of the change.
- **Non-goals**: changing the deploy trigger/auth model; auto-provisioning beyond KV; slimming operators' `wrangler.jsonc` to overlay-only (a follow-up).
