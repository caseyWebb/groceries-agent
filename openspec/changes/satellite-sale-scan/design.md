...
**Satellite rollup replacement + staleness.** A satellite `sale` batch for `(store, locationId)` is the store's full current sale set, so intake **replaces** that store's rollup (`{ store, location_id, as_of: now, items }`) rather than accumulating across cycles — a re-scan supersedes the prior scan, and a partial cannot clobber (one task = one store). Unlike Kroger (a daily cron re-scan bounds staleness), a satellite that goes offline would leave its last rollup indefinitely; so `store_flyer` applies a **read-time staleness ceiling** to a scanned store's rollup (default ~7 days, `operator_config`-tunable): past it, the rollup reads as empty (`items: []`, `as_of` still surfaced) rather than steering menu-gen on stale sales. Not trusting a stale observation indefinitely is itself sensor discipline.
...
## Decision 6 — the sale intake arm (dispatch by `kind`, same raw layer)
...
- `kind: "sale"` → per-item validate + plausibility (Decision 7); drop non-sales (`!isOnSale`) exactly as the Kroger scan does; group the batch by `(store, locationId)`; build `FlyerItem`s **re-deriving `savings`** via the shared helper; **replace** each store's rollup with the freshly-observed set stamped `as_of: now`. Idempotent: a late/double report of the same scan replaces to the same rows (arrival dedup by `productId` within the batch). Per-item dispositions (`accepted`/`rejected`) echo `productId`/`url` in the `ItemResult.source` slot.

`/satellite/results` already runs a claimed task's observations through this shared intake; a `sale-scan` task's `sale` observations flow through the new arm with no channel change — the pull-channel contract treats the payload/results opaquely and is **unmodified**. The `store` + `locationId` come from the **observation** (raw facts), not the task, so intake needs no task context beyond what change 2 already threads.
