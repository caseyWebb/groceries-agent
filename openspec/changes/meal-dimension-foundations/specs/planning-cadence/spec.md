...
**Reason**: Superseded by "The planning window bounds per-meal slot counts, not cooking frequency" — the single `default_cooking_nights` count generalizes to the per-meal `cadence` map (D26-final band-1 scope; stories/02).

**Migration**: `default_cooking_nights` is frozen (the cadence read fallback still reads it for one deprecation window) and drops in the window-close cleanup change.
