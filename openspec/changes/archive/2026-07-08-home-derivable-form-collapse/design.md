...
1. Re-audit selects `lime wedges` → `lime::form-wedges` (un-stamped by 0043). The lexical fast path does not intercept (built over surviving **node ids** only: "lime wedges" folds to `lime wedge`; no node id folds to that — `lime::form-wedges` folds to `lime form wedge`). Cosine retrieval includes `lime` (near neighbor) and always includes the standing survivor; the hardened confirm returns SAME `lime` (well above `NORMALIZE_CONFIRM_MIN`).
