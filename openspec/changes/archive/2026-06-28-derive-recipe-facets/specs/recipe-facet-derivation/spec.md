...
#### Scenario: Derived perishables are normalized

- **WHEN** the classify pass derives `perishable_ingredients` for two recipes that each use fresh cilantro under different wording
- **THEN** both record the same normalized entry, so the two recipes' use of that perishable can be compared directly
