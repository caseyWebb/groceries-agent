...
- Add a session-gated, ETag-aware `GET /api/retrospective/waste` read with the same `4w|8w|12w` range contract and optional mapping-version replay, and extend the existing read-only MCP `retrospective` tool with a shared `waste` object plus optional Waste range/version inputs. The direct member API and UI default to `8w`; the additive MCP inputs default to `4w` and the current mapping version for backward compatibility. Both transports expose the same aggregate object and validation behavior. This change adds no standalone/direct analyzer, derived-value, avoidability, aggregate, edit, delete, or correction writer and does not alter existing `update_pantry` waste-disposition capture, which records qualitative events and accepts no dollar value.
- Replace only the member Retrospective Waste placeholder. Spend and Waste share one canonical `?range=4w|8w|12w` URL control. The Waste panel provides semantic tabs/panels and definition lists, keyboard-operable tab and range groups, textual equivalents for weekly bars and breakdowns, accessible loading status, structured error with retry, distinct empty/unavailable/partial states, and responsive narrow/tall behavior without a chart dependency. Empty current history omits the Tossed-value dollar KPI/breakdowns but retains exact Items binned plus the returned rate and trend. Waste-derived dollar values are labelled as last-paid estimates; any displayed qualifying Spend denominator is labelled as recorded/captured grocery spend, never as a per-toss estimate. Pending Waste department classification is shown separately and does not change complete Waste money from `Last-paid estimate` to a partial label. Unavailable trend amounts are labelled known subtotals and present the returned reason rather than inventing prior-period coverage fields. Most-wasted values follow returned item status directly.
...
- No standalone/direct Waste analyzer, derived-value, avoidability, aggregate, edit, delete, or correction writer; no change to existing qualitative `update_pantry` waste-disposition capture; and no compensation state, historical rewrite, backfill, or silent data repair.
...
Every analyzer arrow is a read. `waste_events` supplies event count, reason, item, `prepared_from`, captured department, and date; `spend_events` supplies only eligible tenant-scoped last-paid values and qualifying grocery spend. No analyzer edge points to a writer, cache fill, queue, scheduled handler, historical backfill, or member-entered value. The separate existing `update_pantry` qualitative disposition-capture path sits outside this read graph and is unchanged.
