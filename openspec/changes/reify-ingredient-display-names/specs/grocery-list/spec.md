...
- **WHEN** `add_to_grocery_list` is called with an explicit `id` of `cabbage::color-red` (e.g. the app materializing an accepted sibling swap)
- **THEN** the row stores `cabbage::color-red` as both its key and its `name` (validated as a live survivor, not re-resolved), it dedups against any existing `cabbage::color-red` row, and it renders the identity node's label ("Red cabbage") resolved at read — never the raw id
