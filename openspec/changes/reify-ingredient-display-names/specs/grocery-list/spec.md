...
- **THEN** the row keys on `cabbage::color-red` directly (validated, not re-resolved), its `display_name` is set from the identity node ("Red cabbage"), and it dedups against any existing `cabbage::color-red` row
