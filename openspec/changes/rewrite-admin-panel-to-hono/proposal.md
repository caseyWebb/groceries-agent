...
- **Rendering: SSR + islands.** Pages are server-rendered with Hono JSX; the genuinely-interactive consoles (Calibration, Tool Console, Table/corpus editors, Members lifecycle) hydrate as **islands**. SSR is a nice-to-have, not a gate — a surface MAY ship islands-only (CSR) first and gain SSR later.
...
- **Build/CI:** `scripts/build-admin.mjs` rewritten (esbuild); `aubr build:admin --check` drift gate preserved; the `package.elm-lang.org` reachability requirement is **removed**. `tsconfig.json` gains `jsx`/`jsxImportSource`. New dependency: `hono` (+ a tiny island runtime — `hono/jsx/dom`, with Preact as the named fallback).
