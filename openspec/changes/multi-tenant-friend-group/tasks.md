...
- [x] 6.4 Idempotent recipe import/discovery: dedupe by source URL/slug so a recipe already in the shared corpus is reused, not duplicated; tests. *(`indexSourceToSlug` source→slug map; `import_recipe` surfaces `existing_slug`, `create_recipe` refuses a duplicate source with `already_exists` (existing slug). Also closed §6.3's leftover: discovery now writes recipes to the SHARED root via `sharedGh`; only `feeds.toml` stays per-tenant.)*

## 7. Shared SKU cache + reference data

- [x] 7.1 Move the SKU cache to the shared corpus; tag each entry with `locationId`; keep per-hit revalidation against the caller's `preferred_location` (fall through to search on mismatch); tests for cross-tenant hit + per-location revalidation. *(SKU cache already read+written via `sharedGh`; added `locationId` to `CachedMapping`/`NewMapping`, write tags new entries with the caller's resolved location, matcher step 2 tries same-location entries first and revalidates every candidate — untagged legacy entries treated as same-location. 4 new matching tests.)*
- [x] 7.2 Shared `aliases`/`ingredients`; `substitutions` shared with an optional per-tenant override layer (override wins for that tenant only); tests. *(`aliases.toml` already shared (sharedGh); `getSubstitutionRules` now joins shared + per-tenant `users/<id>/substitutions.toml` via `mergeSubstitutionRules` (override replaces the shared rule for that ingredient, alias-normalized, that tenant only). 4 new substitution tests. `ingredients.toml` has no reader yet — shared by placement.)*

## 8. Recipe notes (must-build)

- [x] 8.1 Note write tool: append a note to the caller's per-tenant repo (slug, body, optional tags, `private`), recording author (structural) + timestamp; never modify shared content or prior notes; tests. *(`notes.ts` pure helpers + `add_recipe_note` writes `users/<id>/notes/<slug>.toml` via the personal client, append-mostly; author is the subtree path, not a field. Notes stored as TOML `[[notes]]` per the repo's structured-file convention.)*
- [x] 8.2 Note + group-signal reads: aggregate non-private notes (attributed) and group ratings across tenants at read time; exclude others' private notes; cache in KV if needed; tests. *(`TenantStore.list()` (KV `tenant:*` enumeration) → `read_recipe_notes` reads every tenant's notes/overlay via `sharedGh`; `aggregateGroupSignal` includes the caller's own private notes + everyone's shared notes (others' private excluded) and attributed ratings. No KV cache (group is small). 11 notes + 1 directory test.)*
...
- [x] 9.2 Update `docs/PROJECT.md` (multi-tenant architecture, Model B, three-category data model), `docs/SCHEMAS.md` (overlay + note schemas, location-tagged SKU entries, shared vs per-tenant file placement), `docs/TOOLS.md` (read/write contract changes, note tools, group-signal read). *(PROJECT: rewrote Architecture diagram + Repository structure for the code/data split + single private data repo, added multi-tenant identity + three-category-data sections, marked subjective frontmatter per-tenant. SCHEMAS: added shared-vs-per-tenant placement overview, `overlay.toml` + `notes/<slug>.toml` schemas, location-tagged `skus/kroger.toml`, substitutions override layer. TOOLS: `import_recipe`/`create_recipe` idempotency, matcher shared/location cache, substitutions override, `update_recipe` overlay routing, new Recipe note tools section.)*
- [x] 9.3 Update `AGENT_INSTRUCTIONS.md`: surface group ratings/notes; capture spins as notes (not content edits); per-tenant connect + Kroger consent setup. *(Added multi-tenant framing (shared corpus / my-own state / invite-code connect / per-tenant Kroger) to intro, overlay+notes to side-effect files, a "Recipe notes — capture tweaks, don't edit shared content" flow with group-signal surfacing via `read_recipe_notes`, idempotent-import handling (`existing_slug`/`already_exists`), and a never-edit-shared-content-for-a-tweak guardrail.)*
