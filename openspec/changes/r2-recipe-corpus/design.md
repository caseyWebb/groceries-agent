...
- **Deploy substrate** (out of scope but adjacent; needs a decision before any one-click claim). R2 removes the GitHub App, which is what finally makes a "Deploy to Cloudflare" button viable (the App was the step no button could automate — the reason `zero-config-deploy` rejected the button). Two concrete models, an audience call, not a code one:
  - **A — Button + fork (Workers Builds).** The button clones the (public) code repo into the operator's GitHub, provisions KV/D1/R2 from `wrangler.jsonc` (no ids), deploys, and auto-redeploys on push. One-click first run; **no data repo at all** (data in R2). Cost: each operator owns a code fork; updates are a GitHub "Sync fork" that can **conflict** on `wrangler.jsonc` (the `merge-wrangler-config.mjs` allowlist machinery doesn't apply to a raw fork). Friendlier for non-technical self-hosters.
  - **B — Featherweight control repo.** Keep a tiny repo (`wrangler.jsonc` + a `deploy.yml` calling upstream `@main`, no markdown — today's posture minus the corpus). Deploy runs in the operator's Actions, **merges** upstream code-level config (keeps the allowlist, no conflicts), provisions resources, deploys; updates = bump the pinned `code_ref`, no fork to sync. Not one-click, but the cleaner update story and smallest delta from today.
  - Trade: the button optimizes *first-run* at the cost of fork-maintenance + losing the config-merge story; the control repo keeps conflict-free updates at the cost of not being literally one-click. After R2 + the admin panel the *leftover* manual config is tiny either way (Kroger secrets, Access, optional domain — mostly external accounts), so the button's real edge is narrow. A bootstrap-then-detach hybrid exists but is fiddly. **Decide by audience** (non-technical friends → A; GitHub-comfortable → B).
