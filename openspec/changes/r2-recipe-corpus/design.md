...
- **Deploy substrate** (out of scope but adjacent): "Deploy to Cloudflare" button + fork vs. a featherweight control repo (`wrangler.jsonc` + deploy workflow, no markdown). Decide separately.
...
- **Per-author R2 credentials:** scoped R2 tokens per author vs. one operator-managed sync — ties into `obsidian-authoring-vault`.
