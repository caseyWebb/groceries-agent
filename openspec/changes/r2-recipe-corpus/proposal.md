...
- **Out of scope:** the **deploy substrate** ("Deploy to Cloudflare" button + fork vs. a featherweight control repo) — a separate decision; this change moves the *content store*, not how the Worker is deployed. Depends in spirit on `operator-admin-panel` (the `bug_reports` surface, the control-plane half) and benefits from landing after `ai-derived-recipe-metadata` (smaller authored surface, proven reconcile-derive loop).
