...
This repo is the **grocery-agent itself**: the `grocery-mcp` Cloudflare Worker (`src/`), the agent persona/skills source (`AGENT_INSTRUCTIONS.md`, from which `plugin/` is generated), and the build tooling (`scripts/`) that produces the recipe indexes, the static cookbook, and the plugin bundle. The agent *runs* in Claude.ai; it is *built* here. **There is no data in this repo** — each operator's recipes/profile/state live in a separate private data repo + Cloudflare D1/KV (`users/<username>/` is the tenant prefix).
...
- **Don't commit secrets — the repo is public.** Only code lives here; all personal data is in D1 + the private data repo.
