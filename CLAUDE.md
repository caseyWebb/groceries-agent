...
The **operator admin panel** (`/admin`) is an Elm SPA under [`admin/`](admin/) — its modeling standards ("make impossible states impossible", RemoteData, no `Bool`/`Maybe String` state) live in [`admin/CLAUDE.md`](admin/CLAUDE.md). `admin/dist/` is generated — never hand-edit it.
